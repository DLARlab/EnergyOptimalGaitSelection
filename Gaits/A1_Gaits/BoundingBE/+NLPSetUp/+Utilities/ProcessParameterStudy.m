...
%
%
function ProcessParameterStudy(FileName, nlp, info, robot)
    maxTries = 5;
    ...
    if ~exist([FileName,'.mat'],'file')
        ...
        while true % infinite loop, will exit via 'break'
            ...
            velocity = vx;
            nlp.Phase(7).updateConstrProp('average_velocity_entire_stride', 'last','lb', [velocity;0], 'ub', [velocity;0]); 
            nlp.update(); 
            ...
            solver.Options.ipopt.tol                        =  1e-2; %1e-2
            ...
            solver.Options.ipopt.print_level                =  5;
            ...
            if (info.status == 0)
                ...
                costv = checkCosts(nlp, sol);
                COT = costv{7};
                % Store new cost value:                                                            
                configurations{processIndex}.costValue = COT;
