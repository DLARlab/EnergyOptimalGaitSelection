...
addpath(genpath(fullfile(root,'A1_Gaits','BoundingBG'))); 
...
clc;
frost_addpath;
LOAD_PATH = fullfile(root,'A1_Gaits','BoundingBG','gen','sym'); 
EXPORT_PATH = fullfile(root,'A1_Gaits','BoundingBG','gen','opt');
...
if LOAD
    ...
    [sys, domains, guards] = load_behavior(robot, LOAD_PATH,'type','BoundingBG');
else
    robot.configureDynamics('DelayCoriolisSet',OMIT_CORIOLIS,'OmitCoriolisSet',OMIT_CORIOLIS); 
    [sys, domains, guards] = load_behavior(robot,[], 'type','BoundingBG');
end
...
nlp = HybridTrajectoryOptimization('A1_BoundingBG',sys,num_grid,[],'EqualityConstraintBoundary',1e-3);

%% 5- Add Phase Specific Constraints
nlp.Phase(1).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.RRRLConstraint;    
...
nlp.Phase(5).Plant.UserNlpConstraint  = @NLPSetUp.Constraints.FRFLConstraint;     
...
%% 7- Add Multi-Phases Constraints
...
save('GaitLibrary_BG','BG')

%% 12- Animation
close all
gait = BG.V{1}.gait; % Pick a solution (BG.V{i}.gait). 
ANIM_PATH = fullfile(root,'A1_Gaits','BoundingBG','gen','anim');
